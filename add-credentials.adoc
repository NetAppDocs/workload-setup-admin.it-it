...
Scopri di più sulle autorizzazioni necessarie per ogni funzionalità del carico di lavoro in link:permissions-reference.html["Riferimento autorizzazioni"].
...
. Nella pagina Aggiungi credenziali, seleziona *Aggiungi manualmente* e poi segui i passaggi seguenti per completare ogni sezione in _Configurazione autorizzazioni_.
+
...
NOTE: Se hai un account GovCloud, non ti servono le autorizzazioni Cost Explorer  `ce:GetCostAndUsage` o  `ce:GetTags` nella policy **View, planning, and analysis** per Storage. Dopo aver copiato lo script dalla casella di codice, rimuovi queste autorizzazioni ed elimina la virgola finale dalla riga precedente.
...
.Fasi
. Dalla pagina Aggiungi credenziali in Workload Factory, espandi *Crea ruolo* sotto *Configurazione autorizzazioni*.
. Seleziona *codice JSON* per copiare la policy.
. Nella Console di gestione AWS, selezionare *ruoli > Crea ruolo*.
. In *Tipo di entità attendibile*, seleziona *Criterio di attendibilità personalizzato*.
. Sostituisci il contenuto corrente nella casella di testo con il codice JSON che hai copiato al passaggio 2.
. Selezionare *Avanti*.
...
. Torna alla pagina Aggiungi credenziali in Workload Factory e poi incolla l'ARN del ruolo nel campo _Role ARN_.
...
NOTE: L'aggiunta di credenziali a un account AWS GovCloud (USA) tramite CloudFormation non è supportata.
