...
---
= Gestisci gli account di servizio in NetApp Workload Factory
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Facoltativamente, è possibile ottenere un token di accesso per l'API di gestione Auth0 eseguendo uno scambio di credenziali client. Fare riferimento a link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console documentazione API"^] per informazioni sulla generazione di un token di aggiornamento.
+
L'esempio curl mostra come è possibile prendere l'ID client e il segreto e utilizzare un'API per generare il token di accesso, che è limitato nel tempo. Il token fornisce diverse ore di accesso alle API di NetApp Workload Factory.
...
"client_id": "ck2MGBU9LeWCpWlhqyaeximrCmdrNQRm",
"client_secret": "pEleXhAsyf-HXHW5UaFx-Ek1tFT3MKZIEszhVuy6eHePHCwgXkiQT0El_g-WYLUu"
}
----
+
Invia l'output a https://netapp-cloud-account.auth0.com/oauth/token per ottenere il token di accesso.
