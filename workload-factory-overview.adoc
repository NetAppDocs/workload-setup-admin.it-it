...
Connettività link Lambda:: Usa la connettività Lambda link per creare una relazione di fiducia e una connessione tra un account NetApp Workload Factory e uno o più file system Amazon FSx for ONTAP. I link ti permettono di monitorare e gestire alcune funzionalità del file system direttamente dalle chiamate API REST di ONTAP che non sono disponibili tramite l'API Amazon FSx for ONTAP.
...
operazioni di storage EDA:: Semplifica le operazioni di archiviazione per l'automazione della progettazione elettronica (EDA) in NetApp Workload Factory ottimizzando le prestazioni, integrandoti con la tua IaC per automatizzare i parametri di archiviazione e garantendo la salute operativa.
...
NOTE: Gli amministratori dell'account possono attivare o disattivare le funzionalità di intelligenza artificiale. Se un amministratore le disattiva, l'assistente AI Ask Me non sarà disponibile. link:manage-genai-features.html["Scopri come gestire le funzionalità di intelligenza artificiale"].
...
link:https://docs.netapp.com/us-en/workload-fsx-ontap/links-overview.html["Scopri di più sui link"^]
